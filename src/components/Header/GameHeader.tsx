...
import { SoundOnIcon, HintIcon, RestartIcon, CoinIcon } from "../../../public/icons";
        ...
        <div className="relative w-full flex items-center justify-between py-2 sm:py-3 px-2 sm:px-20 gap-2 text-sm sm:text-base">
            ...
            <div className="absolute top-12 sm:static left-1/2 sm:left-0 transform -translate-x-1/2 sm:translate-x-0 w-fit mx-auto flex flex-col items-center justify-center gap-2 sm:gap-3">
                <div className="flex gap-3 sm:gap-2 text-yellow-300 text-3xl sm:text-4xl">
                    {[...Array(3)].map((_, index) => (
                        <FaLightbulb key={index} style={{ color: index < attempts ? "#FFDE05" : "gray" }} />
                    ))}
                </div>
            ...
            <div className="flex items-center gap-2 md:gap-6 sm:gap-3 mt-0 md:mt-[-58px]">
                <button onClick={toggleSound} className="cursor-pointer">
                    {soundOn ? <div className="size-6"><SoundOnIcon /></div> : <FaVolumeOff className="text-gray-800 text-3xl" />}
                </button>
                ...
                <div className="relative size-8 md:size-10 rounded-full bg-[#FFB400] flex items-center justify-center">
                    <CoinIcon />
                    <span className="absolute text-black font-bold text-sm sm:text-base">{coins}</span>
                </div>
            </div>
        </div>
    );
